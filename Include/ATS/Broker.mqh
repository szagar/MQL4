...
public:
  ...
  virtual int       CreateOrder (Position * trade, string comment="");
  ...
  virtual void      DeletePendingTrade(Position * trade);
          int       cntPendingOrders(string symbol, string strategyName);
...
void Broker::closeOpenTrades(string symbol, string strategyName) {
  ...
  for(int x=0;x<OrdersTotal();x++) {
    Info2(__FUNCTION__,__LINE__,"1");
    if(OrderSelect(x,SELECT_BY_POS,MODE_TRADES)) {
      Info2(__FUNCTION__,__LINE__,"2");
      //if(StringCompare(magic.getStrategy(OrderMagicNumber()),strategyName,false)==0) continue;
      Info2(__FUNCTION__,__LINE__,"3");
      if(OrderSymbol()!=Symbol()) continue;
      Info2(__FUNCTION__,__LINE__,"4");
      if(OrderType()==0 || OrderType()==1) {
        Info2(__FUNCTION__,__LINE__,"5");
        Info2(__FUNCTION__,__LINE__,"closeTrade("+string(OrderTicket())+")");
...
void Broker::ExitLong() {
  for(int i=OrdersTotal()-1; i>=0; i--) {
    if ( OrderSelect(i, SELECT_BY_POS) )
      if(OrderSymbol() == Symbol())
        Alert(__FUNCTION__,": WRITE CODE --- exit long here");
  }
}
...
void Broker::SetSLandTP(Position *trade) {
  ...
  if((trade.StopPrice == OrderStopLoss()) &&
    ...
    Warn(trade.Symbol + ": Not sending order to broker because SL and TP already set");
    return;
  }
...
int Broker::CreateOrder (Position * trade, string comment="") {
  Info2(__FUNCTION__,__LINE__,"Entered");
  if (trade.LotSize == 0.0) {
    Warn("Trade with zero lot size cannot be entered.");
    return(-1);
  }
  int tid = OrderSendReliable(symbolPrefix + trade.Symbol + symbolSuffix, 
                    trade.OrderType,
                    trade.LotSize,
                    trade.OpenPrice,
                    0,    //slippage
...
int Broker::cntPendingOrders(string symbol, string strategyName) {
  int cnt = 0;
  Info2(__FUNCTION__,__LINE__,"Orders="+(string)OrdersTotal());
  if (OrdersTotal()==0) return(0);
  for (int i=OrdersTotal()-1; i>=0; i--) {
    if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)==true) {
      //if(StringCompare(magic.getStrategy(OrderMagicNumber()),strategyName,false)==0) continue;
      Info2(__FUNCTION__,__LINE__,"getStrategy(OrderMagicNumber())="+magic.getStrategy(OrderMagicNumber()));
      Info2(__FUNCTION__,__LINE__,"strategyName="+strategyName);
      Info2(__FUNCTION__,__LINE__,"OrderSymbol()="+OrderSymbol());
      Info2(__FUNCTION__,__LINE__,"Symbol()="+Symbol());
      if(StringCompare(OrderSymbol(),Symbol(),false)==0) {
        //if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)==true)
        Info2(__FUNCTION__,__LINE__,"OrderType()="+(string)OrderType());
        if (OrderType()>1 ) cnt++;
      }
    }
  }
...
void Broker::deletePendingOrders(string symbol, string strategyName) {
  ...
  for (int i=OrdersTotal()-1; i>=0; i--) {
    if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)==true) {
      ...
      Info2(__FUNCTION__,__LINE__,"magic.getStrategy(OrderMagicNumber()): "+magic.getStrategy(OrderMagicNumber()));
      Info2(__FUNCTION__,__LINE__,"strategyName: "+strategyName);
      if(StringCompare(magic.getStrategy(OrderMagicNumber()),strategyName,false)==0) {
        Info2(__FUNCTION__,__LINE__,"here1");
        if(StringCompare(OrderSymbol(),Symbol())==0) {
          Info2(__FUNCTION__,__LINE__,"here2");
          Info2(__FUNCTION__,__LINE__,"in: "+OrderSymbol()+" "+string(OrderType()));
          if (OrderType()>1 ) {
            Info2(__FUNCTION__,__LINE__,"here3");
...
void Broker::PrintPendingOrders() {
  int cnt = OrdersTotal();
  Warn2(__FUNCTION__,__LINE__,"Pending Orders: -/"+(string)cnt);
  for (int i=OrdersTotal()-1; i>=0; i--) {
    if (OrderType()<2) continue;
    PrintFormat("Pending: %8s %10d %4.2f %.4f\n",OrderSymbol(),OrderTicket(),OrderLots(),OrderOpenPrice());
  }
}

void Broker::PrintOpenTrades() {
  int cnt = OrdersTotal();
  Warn2(__FUNCTION__,__LINE__,"Open Trades: -/"+(string)cnt);
  for (int i=OrdersTotal()-1; i>=0; i--) {
    if (OrderType()>1) {
      PrintFormat("Open: %d %8s %10d %4.2f %.4f\n",OrderType(),
                  OrderSymbol(),OrderTicket(),OrderLots(),OrderOpenPrice());
    }
  }
}

double Broker::pnlPipsToday() {
    ...
    if (OrderSelect(cc, SELECT_BY_POS) && OrderSymbol() == Symbol()) {
      ...
      if(TimeToStr( OrderOpenTime(), TIME_DATE) == todayTime) {
        if(OrderType() == OP_BUY) {
          ...
          plToday += Bid-OrderOpenPrice();
        } else {
          //plToday += OrderOpenPrice()-sqGetAsk(OrderSymbol());
          plToday += OrderOpenPrice()-Ask;
        }
      }
    }
  }
