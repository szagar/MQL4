...
extern bool Setup_ChannelScalp = false;   //-   Use Channel Scalp setup?
extern bool Setup_TrendFollowing = true; //-   Use Moving Avg setup?
extern bool Setup_MovingAverage = false; //-   Use Moving Avg setup?
extern bool Setup_MovingAvgX = false;    //-   Use Moving Avg Cross setup?
extern bool Setup_Rsi = false;           //-   Use RSI setup?
extern bool UseDefaultParameters = true; //-   Use pre-configured parameters for setup
...
#include <dev\ChannelScalp.mqh>
#include <dev\MovingAverage.mqh>
#include <dev\TrendFollowing.mqh>
...
void Robo::initLongSetupStrategies() {
  ...
  if(Setup_ChannelScalp) {
    Debug4(__FUNCTION__,__LINE__,"Add ChannelScalp Setup");
    setup = new ChannelScalp(Long);
    addLongSetup(setup);
  }

  if(Setup_TrendFollowing) {
    Debug4(__FUNCTION__,__LINE__,"Add TrendFollowing Long Setup");
    setup = new TrendFollowing(Long);
    addLongSetup(setup);
  }
...
void Robo::initShortSetupStrategies() {
  ...
  if(Setup_TrendFollowing) {
    Debug4(__FUNCTION__,__LINE__,"Add TrendFollowing Short Setup");
    setup = new TrendFollowing(Short);
    addShortSetup(setup);  
  }
