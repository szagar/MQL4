...
void HandleClosedTrade(bool savedTrade = false)
{
   ...
   else
   {
     if (!savedTrade)
     {
       ...
       Debug("Realized PIPs   : " + string(RealizedPipsAllPairs) + "\n" 
             "Digits          : " + Digits + "\n"
             "Point           : " + Point + "\n"
//             "Trade diff      : " + diff + "\n"
             "Trade pips      : " + pips + "\n"
...
double PriceLevels_SubtractNlevels(double startLevel,int numLevels) {
  return(startLevel);
}
double PriceLevels_AddNlevels(double startLevel,int numLevels) {
  return(startLevel);
}

void PlotPriceLevelLines() {
  const string RayPrefix="PxLvl_", ArrowPrefix="LvlArrow_";
  string rayName, arrowName;
  ObjectsDeletePrefixed(RayPrefix);
  ObjectsDeletePrefixed(ArrowPrefix);
   ...
   FindDayMinMax(beginningOfDay, TimeCopy[0], TimeCopy, HighPrices, LowPrices);
  // => dayHi, dayLo
  double startLevel = PriceLevels_SubtractNlevels(dayLo,3);
  double endLevel = PriceLevels_AddNlevels(dayHi,3);
  double level = startLevel;
  datetime startTime = beginningOfDay + 16*60*60;
  datetime endTime = startTime + 2*60*60;
  while(level < endLevel) {
    rayName = RayPrefix + string(level);
    arrowName = ArrowPrefix + string(level);
    ObjectCreate(0, rayName, OBJ_TREND, 0, startTime, level, endTime, level);
    ObjectSetInteger(0, rayName, OBJPROP_COLOR, _rangeLinesColor);
    ObjectSet(rayName, OBJPROP_RAY, false);
    ObjectCreate(0, arrowName, OBJ_ARROW_RIGHT_PRICE, 0, endTime, level);
    ObjectSetInteger(0, arrowName, OBJPROP_COLOR, Blue);
  }
}
