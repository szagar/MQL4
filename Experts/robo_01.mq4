...
extern bool FlagWriteFileTradeStats = true;
...
double GetGV(string VarName) {
  string strVarName = VarName;   //StringConcatenate(Prefix,Symbol(),"_",VarName);
...
void CopyInitialConfigVariables()
{
   ...
   _showExit = ShowExit;
   _writeFileTradeStats = FlagWriteFileTradeStats;
      ...
      if(pos > 1)
        {
         ...
         if (var == "NoEntryZoneColor")
         {
            _noEntryZoneColor = StringToColor(value);
                ...
                {
                  _pairOffsetWithinSymbol =  int(StringToInteger(value));
                }
         else if(var == "DefaultStopPips")
                {
                  _defaultStopPips =  int(StringToInteger(value));
                }
                ...
                {
                  _minNoEntryPad = int(StringToInteger(value));
                }
         else if(var == "EntryIndicator")
                {
                  _entryIndicator =  int(StringToInteger(value));
                }
         ...
         else if(var == "FlagWriteFileTradeStats")
                {
               ...
               {
                  _endOfDayOffsetHours = int(StringToInteger(value));
               }
               ...
               {
                  _beginningOfDayOffsetHours =  int(StringToInteger(value));
               }
...
void SaveConfigurationFile()
{
   ...
   FileWriteString(fileHandle, "MinNoEntryPad: " + DoubleToString(_minNoEntryPad) + "\r\n");    // smz
   ...
   FileWriteString(fileHandle, "FlagWriteFileTradeStats: " + IntegerToString((int) _writeFileTradeStats) + "\r\n");
...
void PrintConfigValues()
{
   ...
   Print( "MinNoEntryPad: " + DoubleToString(_minNoEntryPad) + "\r\n");    // smz
   ...
   Print( "FlagWriteFileTradeStats: " + IntegerToString((int) _writeFileTradeStats) + "\r\n");
...
void HandleTradeEntry(bool wasPending, bool savedTrade = false) {
  ...
  if(_cancelPendingTrades && !savedTrade) {
    for(int ix=totalActiveTrades - 1;ix >=0 ;ix--) {
      ...
      if(activeTradesLastTick[ix].IsPending 
         && (activeTradesLastTick[ix].OrderType & 0x01) == (activeTrade.OrderType & 0x01)) {
...
void SetStopAndProfitLevels(Position *trade, bool wasPending) {
  ...
  if (_sendSLandTPToBroker && !_testing) {
    Debug("Sending to broker: TradeType=" + IntegerToString(trade.OrderType) + 
...
void HandleClosedTrade(bool savedTrade = false) {
  if(CheckPointer(activeTrade) == POINTER_INVALID) {
    ...
    if(!savedTrade) {
      ...
      if(_alertOnTrade) {
        Alert("Closed " + IntegerToString (activeTrade.TicketId) + " (" + activeTrade.Symbol + " " + 
              (activeTrade.OrderType == OP_BUY ? "long" : "short") + ") (" + DoubleToStr(tradePnL(activeTrade), 2) + ")");            
      }
      ...
      if((activeTrade.OrderType == OP_BUY && activeTrade.ClosePrice >= activeTrade.OpenPrice) ||
        ...
        if(_showNoEntryZone) {
          ...
          if(ObjectFind(rectName) >= 0) {
            ...
            datetime existingStart = datetime(ObjectGetInteger(0, rectName, OBJPROP_TIME1));  // smz
...
void UpdateGV() {
  ...
  if(GlobalVariableCheck(StringConcatenate(Prefix, normalizedSymbol + "debug"))) {
    int debugFlag = int(GlobalVariableGet(StringConcatenate(Prefix, normalizedSymbol + "debug")));
    debug |= int(debugFlag);
  }
  if(GlobalVariableCheck(StringConcatenate(Prefix,"HeartBeat"))) {
    if(GlobalVariableGet(StringConcatenate(Prefix,"HeartBeat")) == 1)
...
void PlotRangeLines()
{
   ...
   CreatePendingOrdersForRange(dayHi, OP_BUYSTOP, _setPendingOrdersOnRanges, _accountForSpreadOnPendingBuyOrders, int(_marginForPendingRangeOrders), spread);
   CreatePendingOrdersForRange(dayLo, OP_SELLSTOP, _setPendingOrdersOnRanges, _accountForSpreadOnPendingBuyOrders, int(_marginForPendingRangeOrders), spread);
...
void PopulateActiveTradeIds() {
  ...
  for(int ix=0; ;ix++) {
    int tradeId = int(GetGV(MakeGVname(ix+1)));
...
bool CheckForMatchingPendingTrades(Position * newTrade, Position * deletedTrade) {
  ...
  if(deletedTrade.StopPrice == 0.0) {
    if(DEBUG_ORDER) 
      PrintFormat("deleted trade %i has no stop Loss", deletedTrade.TicketId);
    return false;
  }
    ...
    if(newTrade.OrderType <=1) {
      if((deletedTrade.OrderType & 0x01) == (newTrade.OrderType & 0x01)) {
        if(MathAbs(deletedTrade.StopPrice - newTrade.StopPrice) < .000005) {
          ...
          PrintFormat("Stop prices don't match: deletedTrade = %f, newTrade=%f",deletedTrade.StopPrice, newTrade.StopPrice);
      }
      else if (DEBUG_ORDER) 
        PrintFormat("Trades not in same direction: deletedTrade orderType = %i, newTrade orderType = %i", deletedTrade.OrderType, newTrade.OrderType);
    }
    else if (DEBUG_ORDER) 
      PrintFormat("New Trade is not active: orderType = %i", newTrade.OrderType);
  }
  else if (DEBUG_ORDER) 
    PrintFormat("Deleted Trade is not pending: orderType = %i", deletedTrade.OrderType);
  return false;
}
